    ...
    if splitbutton:
        ...
        if bhmgr.empty:
            ...
            for i in range(len(BH_ID)):
                ...
                # Labtest ---------------------------------------------------------------
                if "labtest" in sheetnames:
                    lab_maxcol = shin_lab.max_column
                    lab_maxrow = shin_lab.max_row
                    for j in range(lab_maxcol):
                        wbspt_lab.cell(1,j+1).value = shin_lab.cell(1,j+1).value
                        wbspt_lab.cell(2,j+1).value = shin_lab.cell(2,j+1).value
                        
                        x = 0
                        for k in range(lab_maxrow - 2):
                            if shin_lab.cell(3+k,1).value != BH_ID[i]:
                                pass
                            else:
                                wbspt_lab.cell(3+x,j+1).value = shin_lab.cell(3+k,j+1).value
                                x += 1    
